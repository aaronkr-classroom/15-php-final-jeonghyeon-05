...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  if ($temp and $_FILES['image']['error'] === 0) {
    ...
    $errors['image_file'] .= in_array($ext, $file_extensionns)
  ...
  $errors['summary']       = is_text($article['summary'], 1, 254)
      ? '' : 'Summay must be 1-254 characters';
...
if ($invalid) {
  ...
  try {
      ...
      if ($destination) {
          ...
          $spl = "INSERT INTo image (file, alt)
                  VALUES (:file, :alt);";
